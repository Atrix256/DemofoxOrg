...
inline struct ComplexValue Divide(const struct ComplexValue* A, const struct ComplexValue* B)
{
    struct ComplexValue BConjugate;
    BConjugate.real = B->real;
    BConjugate.imaginary = -B->imaginary;

    struct ComplexValue newA = Multiply(A, &BConjugate);
    struct ComplexValue newB = Multiply(B, &BConjugate);

    struct ComplexValue ret;
    ret.real = newA.real / newB.real;
    ret.imaginary = newA.imaginary / newB.real;
    return ret;
}
